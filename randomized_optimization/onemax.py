...
param_search.plot_ga_param_search(ga_run_stats, problem_name,  outdir=output_dir, limit_iter1=128, limit_iter2=2048)
param_search.plot_sa_param_search(sa_run_stats, problem_name, output_dir, limit_iter1=4096, limit_iter2=8192)
param_search.plot_rhc_param_search(rhc_run_curves, rhc_run_stats, problem_name, output_dir, limit_iter2=4096)
param_search.plot_mimic_param_search(mimic_run_stats, problem_name,  output_dir, limit_iter1=200)
...
sa_best = sa_run_stats[(sa_run_stats['Temperature'] == 100) & (sa_run_stats['schedule_type'] == 'geometric') & (sa_run_stats['Iteration'] <= 4096) ]
rhc_best = rhc_run_stats[(rhc_run_stats['current_restart'] == 9) & (rhc_run_stats['Iteration'] <= 4096) ]
...
for length in lengths_experiment:
    output_dir2 = os.path.join(this_dir, problem_name+'_MIMIC_length', str(length))
    ga_run_stats = pd.read_csv(os.path.join(output_dir2, 'MIMIC', 'mimic__MIMIC__curves_df.csv'))
    max_iteartions_needed.append(ga_run_stats.iloc[ga_run_stats['Fitness'].idxmax()]['Iteration'])
       
ax[2].plot([2**x for x in range(10)], max_iteartions_needed, 'cs--', label='MIMIC best')
   
ax[2].grid()
ax[2].legend()
ax[2].set_xlabel('Problem Length')
ax[2].set_ylabel('Number of Iterations Required')
ax[2].set_title(problem_name+' MIMIC Model Complexity')
