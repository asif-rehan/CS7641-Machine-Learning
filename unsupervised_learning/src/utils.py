...
def tune_hyperparameter(param_grid, pipe, X2_train_nn_reduced, y_train):
    ...
    ypred = tuned_model.predict(X2_train_nn_reduced)
...
def generate_validation_curve(pipe, X2_train_nn_reduced, y_train, model, param_name, search_range, data='wine', log=False):
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=ConvergenceWarning, module="sklearn")
        train_scores, test_scores = validation_curve(estimator=pipe, X=X2_train_nn_reduced, y=y_train,
...
def generate_loss_learning_curve(X2_train_nn_reduced, y_train, model, param_name, search_range, data='wine'):
    plt.close()
    for param_value in search_range:
        ...
        pipe.fit(X2_train_nn_reduced, y_train)
